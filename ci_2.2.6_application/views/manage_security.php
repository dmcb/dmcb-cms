	...
	<?php
		foreach ($functions as $key => $value)
		{
			...
			foreach ($value as $function)
			{
				...
				$i=0;
				foreach ($function['privileges'] as $privilege)
				{
					echo '<td>';
					if (($function['guestpossible'] && $roles[$i]['role'] == "guest") || ($function['ownerpossible'] && $roles[$i]['role'] == "owner") || ($roles[$i]['role'] != "guest" && $roles[$i]['role'] != "owner"))
					{
						...
						echo '<input name="'.$functionfield.'" id="'.$functionfield.'" type="checkbox" class="checkbox" onclick="dmcb.gotovalue(this)" value="'.base_url().'manage_security/setprivilege/'.$function['functionid'].'/'.$roles[$i]['roleid'].'" ';
						if ($privilege)
						{
